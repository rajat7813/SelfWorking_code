...
/*							if (cell.getCellStyle().getDataFormatString().contains("/")) {
			 ...
			 */
			//jRow.put("Key reasons for delay", cells);
			
			if((i>=1)&&(i<=9))
			{
			rows.add(cells);
			}else{
			System.out.println("Not adding to the Json File");
			}
			
			row1.add(tR);
		}
		json.put("the key reason for Delay ", rows);

		FinalA f = new FinalA();
		f.setTrackRem(row1);

		FileWriter fileWriter2 = new FileWriter("D:\\Users\\raarora\\Desktop\\Mydocument\\Sheet_First_trial.json");

		// Writting the jsonObject into sample.json
		fileWriter2.write(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(json));
		
		fileWriter2.append(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(f));
		fileWriter2.close();
